#!/usr/bin/env node
/**
 * Migration runner with legacy-database baseline stamping.
 *
 * Ephemera originally deployed its schema with `drizzle-kit push`, so live
 * databases have all the tables but no `drizzle.__drizzle_migrations`
 * bookkeeping. Running the migrator naively against such a database would
 * try to re-create every table and fail.
 *
 * This script detects that case and "stamps" the baseline migration
 * (0000_baseline) as already applied — inserting the same hash the drizzle
 * migrator would have written — and then lets the normal migrator apply
 * everything after it.
 *
 * Usage: DATABASE_URL=postgres://... node scripts/migrate.mjs
 */
import crypto from "node:crypto";
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import pg from "pg";
import { drizzle } from "drizzle-orm/node-postgres";
import { migrate } from "drizzle-orm/node-postgres/migrator";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const MIGRATIONS_FOLDER = path.resolve(__dirname, "..", "drizzle");

const CONNECT_ATTEMPTS = 30;
const CONNECT_DELAY_MS = 2000;

function log(msg) {
  console.log(`[migrate] ${msg}`);
}

async function connectWithRetry(url) {
  for (let attempt = 1; attempt <= CONNECT_ATTEMPTS; attempt++) {
    const pool = new pg.Pool({ connectionString: url, max: 1 });
    try {
      await pool.query("SELECT 1");
      return pool;
    } catch (err) {
      await pool.end().catch(() => {});
      if (attempt === CONNECT_ATTEMPTS) {
        throw new Error(`Database unreachable after ${attempt} attempts: ${err.message}`);
      }
      log(`waiting for database (${attempt}/${CONNECT_ATTEMPTS})...`);
      await new Promise((r) => setTimeout(r, CONNECT_DELAY_MS));
    }
  }
}

async function tableExists(pool, schema, table) {
  const { rows } = await pool.query(
    `SELECT 1 FROM information_schema.tables WHERE table_schema = $1 AND table_name = $2`,
    [schema, table],
  );
  return rows.length > 0;
}

async function stampBaselineIfNeeded(pool) {
  const hasMigrationsTable = await tableExists(pool, "drizzle", "__drizzle_migrations");
  if (hasMigrationsTable) return;

  const hasLegacySchema = await tableExists(pool, "public", "drops");
  if (!hasLegacySchema) return; // fresh database — migrator will build everything

  const journal = JSON.parse(
    fs.readFileSync(path.join(MIGRATIONS_FOLDER, "meta", "_journal.json"), "utf8"),
  );
  const baseline = journal.entries[0];
  const baselineSql = fs.readFileSync(path.join(MIGRATIONS_FOLDER, `${baseline.tag}.sql`), "utf8");
  // Must match drizzle-orm/migrator.js readMigrationFiles() exactly.
  const hash = crypto.createHash("sha256").update(baselineSql).digest("hex");

  log(`legacy database detected (tables exist, no migration history)`);
  log(`stamping baseline "${baseline.tag}" as applied`);
  await pool.query(`CREATE SCHEMA IF NOT EXISTS drizzle`);
  await pool.query(
    `CREATE TABLE IF NOT EXISTS drizzle.__drizzle_migrations (
       id SERIAL PRIMARY KEY,
       hash text NOT NULL,
       created_at bigint
     )`,
  );
  await pool.query(
    `INSERT INTO drizzle.__drizzle_migrations (hash, created_at) VALUES ($1, $2)`,
    [hash, baseline.when],
  );
}

async function main() {
  const url = process.env.DATABASE_URL;
  if (!url) {
    console.error("[migrate] DATABASE_URL is not set");
    process.exit(1);
  }

  const pool = await connectWithRetry(url);
  try {
    await stampBaselineIfNeeded(pool);
    const db = drizzle(pool);
    await migrate(db, { migrationsFolder: MIGRATIONS_FOLDER });
    const { rows } = await pool.query(
      `SELECT count(*)::int AS n FROM drizzle.__drizzle_migrations`,
    );
    log(`up to date (${rows[0].n} migrations applied)`);
  } finally {
    await pool.end();
  }
}

main().catch((err) => {
  console.error("[migrate] failed:", err);
  process.exit(1);
});
